...
class LakeForm(forms.ModelForm):
    class Meta:
        model = Lake
        fields = ['name', 'location', 'description', 'ecology_info', 'current_status',
                  'cleanup_needs', 'area_sq_km', 'pollution_level']
        widgets = {
            'name': forms.TextInput(attrs={'class': 'form-control'}),
            'location': forms.TextInput(attrs={'class': 'form-control'}),
            'description': forms.Textarea(attrs={'class': 'form-control', 'rows': 4}),
            'ecology_info': forms.Textarea(attrs={'class': 'form-control', 'rows': 4}),
